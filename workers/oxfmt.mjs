...
async function formatViaOxfmt(filename, sourceText, options = {}) {
  ...
  if (normalizedIgnorePath && cwd) {
    const ignorePathPatterns =
      await loadIgnorePathPatterns(normalizedIgnorePath)
    if (ignorePathPatterns.length > 0) {
      const relativePathFromCwd = getRelativePath(cwd, filename)
      const ignorePathMatcher = picomatch(ignorePathPatterns)
  ...
  if (useConfig) {
    ...
    const { overrides: _, ...inlineOptionsWithoutOverrides } =
      inlineFormatOptions
    effectiveOverrides = configOverrides
...
async function loadIgnorePathPatterns(ignorePath) {
  ...
  ignorePathPatternsCache.set(cacheKey, task)
  try {
    return await task
  } catch (err) {
